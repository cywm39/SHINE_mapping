...
class ColorDecoder(nn.Module):
    def __init__(self, config: SHINEConfig, is_time_conditioned = False): 
        ...
        mlp_hidden_dim = config.color_mlp_hidden_dim
        mlp_bias_on = config.color_mlp_bias_on
        mlp_level = config.color_mlp_level

        input_layer_count = config.color_feature_dim
    ...
    def forward(self, feature):
        ...
        output = self.predict_color(feature)
        return output

    # predict the sdf (opposite sign to the actual sdf)
    def predict_color(self, sum_features):
    ...
    def time_conditionded_color(self, sum_features, ts):
